...
class VacationController
{
    ...
    public function index(): void
    {
        ...
        $total = (int)$this->db->prepare("SELECT COUNT(*) FROM vacations v JOIN employees e ON v.employee_id = e.id $wc")->execute($params) ? (int)$this->db->prepare("SELECT COUNT(*) FROM vacations v JOIN employees e ON v.employee_id = e.id $wc")->fetchColumn() : 0;
        // Fix: proper count
        $cs = $this->db->prepare("SELECT COUNT(*) FROM vacations v JOIN employees e ON v.employee_id = e.id $wc");
        $cs->execute($params);
        $total = (int)$cs->fetchColumn();
        $pagination = new Pagination($total, $currentPage);
        $stmt = $this->db->prepare("SELECT v.*, e.full_name AS employee_name, d.name AS department_name FROM vacations v JOIN employees e ON v.employee_id = e.id LEFT JOIN departments d ON e.department_id = d.id $wc ORDER BY v.start_date DESC LIMIT {$pagination->perPage} OFFSET {$pagination->offset}");
        $stmt->execute($params);
    ...
    public function store(): void
    {
        ...
        if (!$data['employee_id'] || !$data['start_date'] || !$data['end_date']) {
            ...
            header('Location: index.php?page=vacations&action=create'); exit;
        }
        ...
        header('Location: index.php?page=vacations'); exit;
    }
    ...
    public function edit(): void
    {
        ...
        if (!$item) { Session::flash('error', 'Nao encontrado.'); header('Location: index.php?page=vacations'); exit; }
        $employees = $this->db->query("SELECT id, full_name FROM employees WHERE status = 'ativo' ORDER BY full_name")->fetchAll();
        View::render('vacations/form', [
    ...
    public function approve(): void
    {
        ...
        $this->db->prepare('UPDATE vacations SET status = ?, approved_by = ?, approved_at = NOW() WHERE id = ?')
    ...
    public function delete(): void
    {
        ...
        Csrf::check();
        $id = Sanitize::int($_POST['id'] ?? 0);
        $this->db->prepare("DELETE FROM schedules WHERE title LIKE ? AND event_type = 'compromisso'")->execute(['Ferias:%' . $id . '%']);
        Vacation::delete($id);
    ...
    public function request(): void
    {
        ...
        $stmt = $this->db->prepare('SELECT employee_id FROM users WHERE id = ?');
        $stmt->execute([$userId]);
        $empId = (int)($stmt->fetchColumn() ?: 0);
        if (!$empId) { Session::flash('error', 'Sem vinculo com funcionario.'); header('Location: index.php?page=my'); exit; }
        $start = Sanitize::date($_POST['start_date'] ?? '');
        $end = Sanitize::date($_POST['end_date'] ?? '');
        if (!$start || !$end) { Session::flash('error', 'Preencha as datas.'); header('Location: index.php?page=my'); exit; }
        if (Vacation::overlapping($empId, $start, $end)) {
            Session::flash('error', 'Voce ja tem ferias no periodo.');
            header('Location: index.php?page=my'); exit;
        }
    ...
    private function syncSchedule(int $vacId, array $data): void
    {
        ...
        $this->db->prepare("DELETE FROM schedules WHERE title LIKE ?")->execute(["Ferias: % [vac:$vacId]"]);
        $stmt = $this->db->prepare('SELECT full_name FROM employees WHERE id = ?');
        $stmt->execute([$data['employee_id']]);
        $name = $stmt->fetchColumn() ?: 'Funcionario';
        $this->db->prepare(
            "INSERT INTO schedules (employee_id, title, event_date, end_time, event_type, color, created_by)
