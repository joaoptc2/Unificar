...
class OnboardingController
{
    ...
    public function store_template(): void
    {
        ...
        if (!$name) { Session::flash('error', 'Nome e obrigatorio.'); header('Location: index.php?m=rh&page=onboarding&action=create_template'); exit; }
        ...
        foreach ($items as $item) {
            ...
            if (!$title) continue;
            $order++;
            $this->db->prepare('INSERT INTO rh_onboarding_template_items (template_id, title, description, responsible_role, sort_order) VALUES (?,?,?,?,?)')
        ...
        header('Location: index.php?m=rh&page=onboarding&action=show&id=' . $tplId); exit;
    }
    ...
    public function assign_store(): void
    {
        ...
        if (!$empId || !$tplId) { Session::flash('error', 'Selecione funcionario e template.'); header('Location: index.php?m=rh&page=onboarding&action=assign'); exit; }
        $tpl = OnboardingTemplate::withItems($tplId);
        if (!$tpl) { Session::flash('error', 'Template nao encontrado.'); header('Location: index.php?m=rh&page=onboarding&action=assign'); exit; }
        $count = 0;
        foreach ($tpl['items'] as $item) {
            $exists = $this->db->prepare('SELECT 1 FROM rh_onboarding_progress WHERE employee_id = ? AND item_id = ?');
            $exists->execute([$empId, $item['id']]);
            if (!$exists->fetchColumn()) {
                $this->db->prepare('INSERT INTO rh_onboarding_progress (employee_id, template_id, item_id) VALUES (?,?,?)')
        ...
        header('Location: index.php?m=rh&page=onboarding&action=progress&employee_id=' . $empId); exit;
    }
    ...
    public function progress(): void
    {
        ...
        $stmt->execute([$empId]);
        $employee = $stmt->fetch();
        if (!$employee) { Session::flash('error', 'Funcionario nao encontrado.'); header('Location: index.php?m=rh&page=onboarding'); exit; }
        $stmt = $this->db->prepare(
            'SELECT p.*, i.title AS item_title, i.description AS item_desc, i.responsible_role,
             ...
             JOIN rh_onboarding_template_items i ON p.item_id = i.id
             JOIN rh_onboarding_templates t ON p.template_id = t.id
    ...
    public function toggle_item(): void
    {
        ...
        $empId = Sanitize::int($_POST['employee_id'] ?? 0);
        $stmt = $this->db->prepare('SELECT completed FROM rh_onboarding_progress WHERE id = ?');
        $stmt->execute([$progressId]);
        $current = $stmt->fetchColumn();
        if ($current === false) { header('Location: index.php?m=rh&page=onboarding'); exit; }
        $newVal = (int)$current ? 0 : 1;
        $this->db->prepare('UPDATE rh_onboarding_progress SET completed = ?, completed_by = ?, completed_at = ? WHERE id = ?')
    ...
    public function delete_template(): void
    {
        ...
        header('Location: index.php?m=rh&page=onboarding'); exit;
    }
}
