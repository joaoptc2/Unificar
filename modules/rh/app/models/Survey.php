...
class Survey extends Model
{
    ...
    public static function withQuestions(int $id): ?array
    {
        ...
        $stmt = self::db()->prepare('SELECT * FROM rh_survey_questions WHERE survey_id = ? ORDER BY sort_order');
        $stmt->execute([$id]);
    ...
    {
        $stmt = self::db()->prepare('SELECT COUNT(DISTINCT IFNULL(user_id, id)) FROM rh_survey_responses WHERE survey_id = ?');
    ...
    public static function results(int $surveyId): array
    {
        $stmt = self::db()->prepare(
            'SELECT q.id, q.question, q.type, AVG(r.rating) AS avg_rating, COUNT(r.id) AS total_responses
             FROM rh_survey_questions q
             LEFT JOIN rh_survey_responses r ON r.question_id = q.id
             WHERE q.survey_id = ?
