...
class Announcement extends Model
{
    ...
    {
        $sql = "SELECT a.*, u.name AS author_name FROM announcements a LEFT JOIN users u ON a.created_by = u.id WHERE a.published_at IS NOT NULL AND a.published_at <= NOW() AND (a.expires_at IS NULL OR a.expires_at >= CURDATE())";
        $params = [];
    ...
    {
        self::db()->prepare('INSERT IGNORE INTO announcement_reads (announcement_id, user_id) VALUES (?, ?)')->execute([$announcementId, $userId]);
    }

    public static function isRead(int $announcementId, int $userId): bool
    {
        $stmt = self::db()->prepare('SELECT 1 FROM announcement_reads WHERE announcement_id = ? AND user_id = ?');
