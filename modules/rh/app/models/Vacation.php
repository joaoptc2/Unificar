...
class Vacation extends Model
{
    ...
    {
        $stmt = self::db()->prepare('SELECT * FROM vacations WHERE employee_id = ? ORDER BY period_start DESC');
        $stmt->execute([$empId]);
    ...
    {
        $sql = 'SELECT 1 FROM vacations WHERE employee_id = ? AND status NOT IN ("rejeitada","concluida") AND start_date <= ? AND end_date >= ?';
