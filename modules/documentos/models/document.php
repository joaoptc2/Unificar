...
function _doc_has_workflow() {
    ...
    if ($_doc_has_status === null) $_doc_has_status = db_has_column('doc_documents', 'status');
...
function document_filter_where($hospital_id, $filter, $search, $category, $status = '') {
    ...
    if ($filter === 'expired') {
        ...
        if (db_has_column('doc_documents', 'next_review_date')) {
...
function document_list($hospital_id, $filter = 'all', $search = '', $category = '', $status = '', $limit = 20, $offset = 0) {
    ...
    $params[] = (int) $offset;
    $rows = db_query(
        "SELECT * FROM doc_documents" . $where . " ORDER BY expiration_date ASC LIMIT ? OFFSET ?",
        $params
    );
...
function document_distinct_categories($hospital_id) {
    return db_query(
        "SELECT DISTINCT category FROM doc_documents
...
function document_create($hospital_id, array $data, array $file_data = null, $created_by = null) {
    ...
    foreach ($opt_cols as $c) {
        if (db_has_column('doc_documents', $c) && array_key_exists($c, $data)) {
            $cols[] = $c;
            $vals[] = $data[$c];
        }
    }
...
function document_mark_reviewed($id, $hospital_id, $interval_months = 12) {
    if (!db_has_column('doc_documents', 'last_reviewed_at')) return 0;
    return db_execute(
...
function document_stats($hospital_id) {
    ...
    $rows = db_query(
        "SELECT COUNT(*) AS total,
         ...
         FROM doc_documents WHERE hospital_id = ? AND deleted_at IS NULL",
    ...
    if (_doc_has_workflow()) {
        try {
            $r = db_query_one("SELECT
                ...
                FROM doc_documents WHERE hospital_id = ? AND deleted_at IS NULL", [$hospital_id]);
            if ($r) {
    ...
    if (db_has_column('doc_documents', 'next_review_date')) {
        try {
            $r = db_query_one("SELECT COUNT(*) AS c FROM doc_documents
...
function document_review_pending_list($hospital_id, $limit = 10) {
    if (!db_has_column('doc_documents', 'next_review_date')) return [];
    return db_query(
...
function document_save_version($document, $notes = '', $user_id = null) {
    if (!db_has_table('doc_document_versions')) return false;
...
function document_bump_version($id, $hospital_id) {
    if (!db_has_column('doc_documents', 'current_version')) return;
...
function document_versions($document_id) {
    if (!db_has_table('doc_document_versions')) return [];
    try {
        return db_query(
...
function document_version_find($version_id, $document_id) {
    if (!db_has_table('doc_document_versions')) return null;
    return db_query_one(
...
function document_acknowledge($document_id, $user_id, $ip = null) {
    if (!db_has_table('doc_document_acknowledgments')) return false;
    try {
        db_execute(
...
function document_acknowledgment_status($document_id) {
    if (!db_has_table('doc_document_acknowledgments')) return ['acknowledged' => [], 'total_users' => 0];
    try {
        $acks = db_query(
...
function document_categories_list() {
    if (!db_has_table('doc_document_categories')) return [];
    try {
