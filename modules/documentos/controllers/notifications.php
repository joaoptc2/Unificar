...
function notifications_index($param = null) {
    ...
    $total = 0;
    try {
        $total      = notification_count($hospital_id, $user_id, $filter);
        $pagination = paginate($total);
        $notifications = notification_list($hospital_id, $user_id, $filter,
...
function notifications_read($param = null) {
    ...
    try {
        notification_mark_read($id, get_user_id(), get_hospital_id());
    } catch (Exception $ex) {
...
function notifications_readall($param = null) {
    ...
    csrf_validate();
    try {
        notification_mark_all_read(get_user_id(), get_hospital_id());
...
function notifications_delete($param = null) {
    ...
    $id = sanitize_int(input('id'));
    try {
        notification_soft_delete($id, get_user_id(), get_hospital_id());
    } catch (Exception $ex) {
