...
class Process extends Model
{
    ...
    public static function active(): array
    {
        ...
        $stmt = $db->query(
            'SELECT p.*, u.name AS creator_name,
                    (SELECT COUNT(*) FROM process_steps ps WHERE ps.process_id = p.id) AS total_steps,
                    (SELECT COUNT(*) FROM process_steps ps WHERE ps.process_id = p.id AND ps.status = "completed") AS completed_steps
             FROM processes p
             LEFT JOIN users u ON u.id = p.created_by
    ...
    public static function addStep(int $processId, array $data): int
    {
        $db = Database::getInstance();
        $stmt = $db->prepare(
            'INSERT INTO process_steps (process_id, title, description, order_num, assigned_to, due_date)
             VALUES (?, ?, ?, ?, ?, ?)'
        );
        $stmt->execute([
    ...
    public static function updateStepStatus(int $stepId, string $status, ?int $userId = null): void
    {
        ...
        $db->prepare(
            'UPDATE process_steps SET status = ?, completed_at = ?, completed_by = ? WHERE id = ?'
    ...
    {
        $db = Database::getInstance();
        $stmt = $db->prepare('SELECT COUNT(*) FROM process_steps WHERE process_id = ?');
        $stmt->execute([$processId]);
        $total = (int) $stmt->fetchColumn();

        if ($total === 0) {
            $db->prepare('UPDATE processes SET progress = 0 WHERE id = ?')->execute([$processId]);
            return;
        }

        $stmt = $db->prepare('SELECT COUNT(*) FROM process_steps WHERE process_id = ? AND status = "completed"');
        $stmt->execute([$processId]);
        $done = (int) $stmt->fetchColumn();

        $progress = (int) round(($done / $total) * 100);
        $db->prepare('UPDATE processes SET progress = ? WHERE id = ?')->execute([$progress, $processId]);
    }
}
