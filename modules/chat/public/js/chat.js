...
class TeamChat {
    ...
    initPresence() {
        ...
        window.addEventListener('beforeunload', () => {
            navigator.sendBeacon('index.php?page=api&action=userStatus', new URLSearchParams({
    ...
    notifyBrowser(title, body) {
        if ('Notification' in window && Notification.permission === 'granted' && document.hidden) {
            new Notification(title, { body: body, icon: 'public/css/favicon.ico' });
        }
    }
    ...
    async api(action, method, data) {
        let qs = 'page=api&action=' + encodeURIComponent(action);
        ...
        try {
            ...
            if (res.status === 401) { window.location.href = 'index.php?page=login'; return null; }
    ...
    async searchUsersForDm(query) {
        ...
        container.innerHTML = data.users.map(u =>
            `<a href="index.php?page=channels&action=direct&user_id=${u.id}" class="user-result-item">
                <span class="dm-status ${this.esc(u.status)}"></span>
